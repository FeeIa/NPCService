...
export type SimplePath = {
	new: (agent: Model, agentParameters: {[string]: any?}, override: {}) -> SimplePath,
	Destroy: (self: SimplePath) -> (),
	Stop: (self: SimplePath) -> (),
	Run: (self: SimplePath, target: Vector3 | BasePart) -> (),
	Visualize: boolean,
	StatusType: {
		Idle: string,
		Active: string,
