...
local NPCService = {}
NPCService.NPCControllers = {} :: {Types.NPCController}
...
---- PUBLIC APIs
-- Initialize existing NPC models in Workspace with behaviourConfig according to tags and default pathConfig
function NPCService:Init()
	for _, NPC: Model in pairs(game.Workspace:GetDescendants()) do
		if NPC:IsA("Model") and NPC:FindFirstChildOfClass("Humanoid") 
			and NPC:HasTag(SETTINGS.NPC_TAG) then
			
			local behaviourConfig = {
				isAggressive = NPC:HasTag(SETTINGS.NPC_AGGRESSIVE_TAG),
				canRespawn = NPC:HasTag(SETTINGS.NPC_RESPAWN_TAG),
				respawnAtOrigin = NPC:HasTag(SETTINGS.NPC_RESPAWN_AT_ORIGIN_TAG),
				canWander = NPC:HasTag(SETTINGS.NPC_WANDER_TAG)
			}
			
			NPCService:AddNPC(NPC, behaviourConfig)
		end
	end
end
